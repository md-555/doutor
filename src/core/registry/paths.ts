...
export const DOUTOR_DIRS = {
  ...
  STATE: path.join(PROJECT_ROOT, '.doutor'),

  /** Diretório de histórico de métricas (.doutor/historico-metricas/) */
  METRICS_HISTORY: path.join(PROJECT_ROOT, '.doutor', 'historico-metricas'),
...
export const REPORT_PATTERNS = {
  ...
  DIAGNOSTICO: (timestamp: string) =>
    path.join(DOUTOR_DIRS.REPORTS, `doutor-diagnostico-${timestamp}.md`),

  /** Relatório JSON resumo (doutor-relatorio-summary-{timestamp}.json) */
  SUMMARY_JSON: (timestamp: string) =>
    path.join(
      DOUTOR_DIRS.REPORTS,
      `doutor-relatorio-summary-${timestamp}.json`,
    ),
  ...
  PERF_BASELINE: (timestamp: string) =>
    path.join(DOUTOR_DIRS.PERF, `baseline-${timestamp}.json`),
...
export const MIGRATION_MAP = {
  ...
  [DOUTOR_FILES.GUARDIAN_BASELINE_LEGACY]: DOUTOR_FILES.GUARDIAN_BASELINE,

  // Estrutura: baseline-estrutura.json → estrutura.baseline.json
  [DOUTOR_FILES.ESTRUTURA_BASELINE_LEGACY]: DOUTOR_FILES.ESTRUTURA_BASELINE,

  // Arquétipo: doutor.repo.arquetipo.json (raiz) → .doutor/estrutura.arquetipo.json
  [DOUTOR_FILES.ESTRUTURA_ARQUETIPO_LEGACY_ROOT]:
    DOUTOR_FILES.ESTRUTURA_ARQUETIPO,
} as const;
 ...
 */
export type DoutorFilePath =
  (typeof DOUTOR_FILES)[keyof typeof DOUTOR_FILES];
