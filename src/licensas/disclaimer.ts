  ...
  try {
    ...
    async function walk(dir: string): Promise<string[]> {
      ...
      for (const e of entries) {
        ...
        if (e.isDirectory()) {
          if (/^(node_modules|dist|\.git|pre-public|preview-sensei|coverage|relatorios|\.sensei)$/i.test(e.name)) continue;
...
export async function verifyDisclaimer({
  ...
  const files = (await listMarkdown(root)).filter(f => f !== disclaimerPath && !f.startsWith('pre-public/') && !f.startsWith('preview-sensei/')).filter(f => !f.startsWith('.abandonados/') && !f.startsWith('.deprecados/') && !f.startsWith('coverage/') && !f.startsWith('relatorios/'));
