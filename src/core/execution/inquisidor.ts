...
export async function iniciarInquisicao(
  ...
  if (
    ...
    try {
      ...
      if (config.DEV_MODE)
        log.erro(`Falha priorização: ${(e as Error).message}`);
    }
  }
  ...
  if (!skipExec) {
    log.sucesso(
      `🔮 Inquisição concluída. Total de ocorrências: ${ocorrencias.length}`,
    );
  } else if (!config.COMPACT_MODE) {
    __infoDestaque(
      `Varredura concluída: total de arquivos: ${fileEntries.length}`,
    );
  }
