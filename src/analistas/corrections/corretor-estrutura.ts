...
export async function corrigirEstrutura(
  ...
  await Promise.all(
    mapa.map((entry) =>
      limit(async () => {
        ...
        if (!AUTO_FIX) {
          log.info(
            `→ Simular: ${arquivo} → ${path.relative(baseDir, destino)}`,
          );
          return;
        }
        ...
        try {
          ...
          const msg =
            err && typeof err === 'object' && 'message' in err
              ? String((err as ErroComMensagem).message)
              : String(err);
        ...
        try {
            ...
            if (config.SAFE_MODE && !config.ALLOW_MUTATE_FS) {
              ...
              const { novoConteudo } = reescreverImports(
                ...
                path.posix.normalize(
                  path.relative(baseDir, destino).replace(/\\/g, '/'),
                ),
              );
          ...
          log.sucesso(
            `✅ Movido: ${arquivo} → ${path.relative(baseDir, destino)}`,
          );
        } catch (err) {
          const msg =
            err && typeof err === 'object' && 'message' in err
  ...
  for (const pluginRel of PLUGINS) {
    try {
      ...
      const pluginModule: unknown = await importarModuloSeguro(
        baseDir,
        String(pluginRel),
      );
      let pluginFn:
