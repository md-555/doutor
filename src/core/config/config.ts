...
async function carregarArquivoConfig(): Promise<Record<
  string,
  unknown
> | null> {
 ...
 */
function converterConfigSimplificada(
  ...
  if (config.languages && typeof config.languages === 'object') {
    ...
    const langDefaults = {
      ...
      css: { parser: 'css-tree', plugin: 'core' },
    ...
    for (const [lang, enabled] of Object.entries(langs)) {
      const defaults = langDefaults[lang as keyof typeof langDefaults];
      if (defaults) {
...
function sincronizarIgnorados() {
  ...
  const glob = Array.isArray(dyn.globalExcludeGlob)
    ? dyn.globalExcludeGlob
    : [];
  ...
  delete (config as unknown as Record<string, unknown>)
...
export async function inicializarConfigDinamica(
